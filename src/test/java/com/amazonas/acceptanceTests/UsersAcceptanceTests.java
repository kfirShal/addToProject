...
//-------------------------UseCase 2.1.3-------------------------
    ...
    void testRegisterFailureUsernameAlreadyExists() throws UserException {
        ...
        doThrow(new UserException("Registration failed - Username Already Exists")).when(usersController).register(email, userName, password);
        String result = userProfilesService.register(request.toJson());
        assertEquals(Response.getErrorResponse(new UserException("Registration failed - Username Already Exists")).toJson(), result);
    }
//-------------------------UseCase 2.1.4-------------------------
    @Test
